...
class UsintArray(templates.NumericArray):
    ...
    def type_validator(v: Any):
        if not isinstance(v, list):
            raise TypeError(
                f"USINTARRAY default values must be a list of int, got {type(v).__name__!r}: {v!r}"
            )
        if len(v) == 0:
            raise ValueError(f"USINTARRAY values must not be empty")
    ...
    def _(name: str, preset):
        return [(f"{name}", TypeSpec(f"USINT[{len(preset)}]", list(preset)))]
